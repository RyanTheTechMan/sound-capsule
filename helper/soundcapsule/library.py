...
class CapsuleLibrary:
                    ...
                    database.execute(
                        ...
                        (
                            ...
                            json.dumps(self._note_preview(
                                capsule, manifest.channels,
                                manifest.source_pattern_length_steps, manifest.source_ppq,
                            )),
                            modified_ns,
                        ),
                    )
    ...
    def _note_preview(
        ...
        pattern_end = round(pattern_length_steps * source_ppq / 4) if pattern_length_steps else 0
    ...
    def rename(self, capsule_id: str, name: str) -> None:
        with self._lock:
            capsule = self.find(capsule_id)
            self._rewrite_manifest(capsule, name=name)
            self.reindex()
